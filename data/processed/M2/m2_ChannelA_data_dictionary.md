...
> 原始 patch：`data/raw/02_sentinel2/Channel A/s2_patches/`（GeoTIFF，**不入 git**）
> Manifest：`data/raw/02_sentinel2/Channel A/s2_patches/S2_patches_manifest_ALL.csv`
> 审计 / 索引 / 嵌入产物：`data/processed/M2/outputs/s2_patch_*.csv`、`s2_prithvi_emb_*`
> GEE 导出脚本：`data/raw/02_sentinel2/Channel A/export_s2_patches_multimodal_gee{,_bundled}.js`
> 本地脚本：`data/processed/M2/py/audit_s2_patches.py`、`s2_patch_utils.py`、`precompute_s2_embeddings.py`
> 配套 Channel B 机制表：`data/processed/M2/m2_ChanelB_data_dictionary.md`
...
```
GEE 导出 (Channel A/*.js)
  └─► data/raw/.../s2_patches/*.tif + S2_patches_manifest_ALL.csv
        └─► audit_s2_patches.py
...
```bash
python3 data/processed/M2/py/audit_s2_patches.py
```
...
```bash
# 冒烟（4 张）
python3 data/processed/M2/py/precompute_s2_embeddings.py --limit 4
# 全量（963 张）
...
- **RAW patch 不入 git**：`data/raw/` gitignore；换机需从 GEE Drive 文件夹 `CASA0004_S2_patches` 重新同步至 `Channel A/s2_patches/`。
...
| 路径 | 内容 |
| --- | --- |
| `data/raw/02_sentinel2/Channel A/s2_patches/` | GeoTIFF（968 文件，~2 GB） |
| `data/raw/02_sentinel2/Channel A/s2_patch_exclusions.csv` | 手工无效 patch 清单 |
| `data/raw/02_sentinel2/aoi_oil_infrastructure.csv` | 11 站坐标与类型（Channel B 5 km 缓冲亦用此表） |
| `data/processed/M2/outputs/s2_patch_index.csv` | 全量 (site, month) 有效性索引 |
| `data/processed/M2/outputs/s2_prithvi_emb_meanpool.npy` | 主 embedding 数组 |
| `data/processed/M2/m2_ChanelB_data_dictionary.md` | Channel B 机制特征字典 |
