...
├── 05_outputs/                        # 基准实验输出（baselines/Flat + Deep）
...
├── scripts/
│   ├── flat/
│   │   ├── run_baseline.py            # 扁平特征基准（Ridge / XGB / M0–M4）
│   │   ├── M1_Flat/sweep_m1.py
│   │   ├── M2_Flat/{sweep,robustness,shap}_m2.py
│   │   ├── M3_Flat/{sweep,robustness,shap}_m3.py
│   │   └── M4_Flat/{sweep,robustness,shap}_m4.py
│   ├── deep/
│   │   ├── run_deep_baseline.py       # 深度融合主入口
│   │   ├── run_deep_{sweep,interpret,advanced,fusion_matrix,xattn_viz}.py
│   │   ├── run_deep_interpret_m3.py
│   │   └── {diagnose_rs,diagnose_deep,compare_rs_anom,multiseed_rs_anom}.py
│   └── tools/
│       ├── subperiod_eval.py
│       ├── migrate_model_names.py
│       └── relocate_deep_outputs.py
├── src/
│   ├── backtest/                      # 滚动回测框架（Flat）
│   └── models/                        # 深度编码器 + 融合 + deep_rolling
└── notebooks/                         # 空（占位）
```

**说明：** Flat 与 Deep 脚本分目录；路径辅助见 `04_code/src/model_naming.py`。
...
└── baselines/
    ├── Flat/
    │   ├── M1_Flat/                   # 金融基准
    │   ├── M2_Flat/                   # 遥感（anom / literature / LOAO 等变体）
    │   ├── M3_Flat/                   # 航运
    │   └── M4_Flat/                   # 扁平多模态融合
    ├── Deep/
    │   ├── M1_Deep/                   # 金融 TCN 编码器
    │   ├── M2_Deep/                   # fin+RS 融合 + RS 消融
    │   ├── M3_Deep/                   # fin+航运融合 + 门控解释
    │   ├── M4_Deep/                   # 全模态融合 + interpret/xattn/advanced
    │   └── _cross/                    # 跨模态汇总（metrics / predictions / sweep / fusion_matrix）
    └── subperiod/                     # 子期评估（Flat + Deep）
```
...
03_data/processed/{M1,M2,M3,merge}
        ↓
04_code/scripts/  ──→  05_outputs/baselines/Flat/M*_Flat/ + Deep/
        ↓
06_writing
        ↑
