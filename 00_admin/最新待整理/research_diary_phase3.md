...
- **【已实现 opt-in；多 seed 已跑；结论=保留为补充实验，不升主模型】修"图像变化太弱"：RS 嵌入站内去均值异常化**。已实现 `deep_dataset._site_expanding_demean` + `rs_kind="meanpool_anom"`（月度层 past-only expanding 去均值再 as-of，泄漏安全；理论支点 [P032]、与 Channel B `anom` 口径一致）。
  ...
  - **多 seed（5 个：42,0,1,2,3）**（`multiseed_rs_anom.py`；日志 `05_outputs/baselines/deep/rs_anom_multiseed.{log,csv}`）：`m4rep` anom>raw 在 **4/5** seed，Δskill 均值 **+0.54%**、方差降低（std 0.70→0.47）；**但** anom 均值 skill 仍 **−0.54%（未过 M0，仅 seed42 过）**、DirAcc 几乎不变（48.5→48.9）、**配对 t p=0.168（不显著）**。
