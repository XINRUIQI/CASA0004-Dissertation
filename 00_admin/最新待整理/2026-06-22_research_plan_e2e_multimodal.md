...
- **核心实证层（baseline / ablation）**：现有工程特征 + XGBoost / 线性，做稳健、可复现的 M0–M4 消融。
- **方法集成与实证检验层（contribution）**：不提出新的融合算子 / 网络层 / 损失函数，而是把模态专属编码器 + 表示级融合（含冻结 EO 基础模型表示、门控 / 交叉注意力、缺失模态与异步时间建模）**集成**为一个连贯系统，并**首次**在原油周频价格预测中、于统一无泄漏协议 + DM/Clark–West 下，系统检验「保留模态结构的表示级融合是否优于把所有数据压成一张表」。贡献性质为 **application + integration + 系统实证比较**，非方法学创新。
...
> **贡献定位：** 本研究所用的模态编码器、门控 / 交叉注意力融合、冻结 EO 基础模型、缺失模态与不规则时间建模**均为既有方法**；新意不在于提出新算法，而在于其**面向原油市场的适配与组合**，以及对上述实证问题的**首次系统、可复现检验**。因此贡献类型定位为 **application + integration + 系统实证比较**，而非方法学突破——这与导师一贯强调的「公平、可复现比较」一致。
...
- **RQ2（方法集成 / 实证比较）**：在相同数据下，**模态感知表示级融合**（模态专属编码器 + 门控交叉注意力）是否优于**扁平特征融合**（所有数值列拼一张表喂 XGBoost / 线性）？
...
- 辅助评估指标（由预测价格派生，非独立预测任务）：方向  \mathrm{sign}(\hat P_{t+1}-P_t) 、收益率，仅用于评估与可解释性。————暂时不做
...
预测起点统一为每周五收盘，预测目标为下一周。统一比较窗 **2019.1–2025.12**（M1/M2/M3 数据均覆盖至 2025-12，窗内每周模态齐备且 target 完整）；最佳模型可在更长历史（全量 union 2006–2026，见 merge 脚本 `--full`）做第二阶段稳健性测试。
...
### 4.2 遥感模态（Earth Observation Modality）
...
- **双通道设计**（表示学习 + 经济解释并存）：
  ...
  - 通道 B — 机制变量：保留现有 NDVI/NDWI/NDBI/BSI + NTL 共 5 个人工指标
...
- 必含 **M0（随机游走  \hat P_{t+1}=P_t ）** 作为主基准。
...
#### 6.2.1 扁平对照基线Flat baseline

作为 M0–M4 的统一**扁平特征融合**对照（后续加模态 M2/M3/M4 与方法集成 / 实证检验层都以此为标尺）：
