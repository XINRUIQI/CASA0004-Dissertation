...
- SHAP：`m2/shap_m2.py` 固定 holdout（train≤2023-12，test=2024-01–2025-12）；按 RS 指数与 AOI 分组；产物 `05_outputs/baselines/Flat/M2_Flat/shap_*.csv` + `shap_anom.png`。
...
- `04_code/scripts/flat/run_baseline.py` — 输出路由至 `baselines/m1|m2|m3/`
- `04_code/scripts/flat/M1_Flat/sweep_m1.py`（自 `sweep_baseline.py` 迁入）
- `04_code/scripts/flat/M2_Flat/shap_m2.py` / `robustness_m2.py` / `sweep_m2.py`
- `04_code/scripts/flat/M3_Flat/sweep_m3.py` / `robustness_m3.py` / `shap_m3.py`（骨架）
- `05_outputs/baselines/Flat/M1_Flat/` / `m2/` — 产物迁移 + SHAP + C2
...
- 新建 `04_code/scripts/deep/run_deep_baseline.py`：LSTM/GRU **early-fusion** 时序基线，复用 `backtest.data`（同特征/窗口/防泄漏）+ `backtest.metrics`（同 DM/CW），把所选全部数值列 reshape 成 `[lookback, features]` 序列喂进**一个共享 RNN**（无模态专属编码器）——即 RQ2 里「扁平 / 早融合」的**深度版标尺**（此前只有 Ridge/XGB 的表格扁平融合，缺深度 early-fusion）。
...
- 产物：`05_outputs/baselines/Deep/`（`M*_Deep/` + `_cross/`） 下 `deep_metrics.csv`（含 DM/CW）+ `deep_cw.csv` + `deep_predictions.csv` + `deep_backtest.png`（单一 `deep/` 目录，非 `flat/M{1..4}_Flat/` 分目录）。
...
> ⚠️ **产物注记（2026-07-05 补记）**：上表为 07-03 当天 LSTM early-fusion 的历史结果。此后 `04_code/scripts/deep/run_deep_baseline.py` 与 `05_outputs/baselines/Deep/` 已被 2026-07-05 方法集成层工作覆盖——当前 `deep_metrics.csv` 内容已是模态感知融合架构 `Mfin_TCN / Mship_GNN / Mrs_RS / Mfusion / Mfull_M4rep`（M0 RMSE=4.172，测试样本对齐略有差异），LSTM 原始产物需回溯 git 历史复现。详见下方 2026-07-05 条目。
...
- 在 `backtest/data.py` 新增 `--m2-features aoi4|ntlall` 后重跑两个 M2 稀疏合约（同 L4_tuned、257 测试周）：`aoi4` = 站点精选（Fujairah / RasTanura / Rotterdam / Houston 4 核心枢纽 × 5 指数 anom = **20 列**）、`ntlall` = 指数精选（11 AOI 仅 `NTL_anom` = **11 列**）。产物 `05_outputs/baselines/Flat/M2_Flat/baseline_metrics_{aoi4,ntlall}.csv`。
...
- 另建 `run_deep_interpret.py`（RQ3 门控/节点注意力可视化）与 `04_code/scripts/deep/run_deep_sweep.py`（多 seed / 超参稳健性）脚手架（git 未跟踪）。
- **本管线产物覆盖了 07-03 的 LSTM early-fusion**（`baseline_deep_*` 已删），当前 `05_outputs/baselines/Deep/_cross/deep_metrics.csv` 为下表。
...
- 新增创新层：`04_code/src/models/*.py`（3 编码器 + `GatedFusion` + dataset/rolling）、`04_code/scripts/deep/run_deep_baseline.py`（LSTM 标尺重写为融合入口）+ `04_code/scripts/deep/run_deep_interpret.py` + `04_code/scripts/deep/run_deep_sweep.py`、`03_data/processed/M3/py/build_m3_graph17.py`（+ `m3_graph17_*` 产物）。
...
- 覆盖重跑：`05_outputs/baselines/Flat/M*_Flat/ + Deep/*`。
...
- [x] 扁平 M0/M1 基线回测 + lookback 稳健性 + 调优 sweep；锁定 `L4_tuned` 对照（`00_admin/待整理/flat_baseline_log.md`、`05_outputs/baselines/Flat/M1_Flat/`）
...
- [x] 深度 early-fusion 基线（LSTM，M1–M4）✅ 2026-07-03 `04_code/scripts/deep/run_deep_baseline.py`（RQ2 扁平标尺）
...
python 04_code/scripts/flat/run_baseline.py \
  --modality M2 \
  --m2-features-csv 03_data/processed/M2/outputs/m2_weekly_features_watermask.csv \
  --out 05_outputs/baselines/Flat/M2_Flat/watermask/
```
...
> 证据来源：`04_code/scripts/deep/diagnose_rs.py` → `05_outputs/baselines/Deep/M2_Deep/rs_diagnostic.log`。
...
- **【已实现 opt-in；多 seed 已跑；结论=保留为补充实验，不升主模型】修"图像变化太弱"：RS 嵌入站内去均值异常化**。已实现 `deep_dataset._site_expanding_demean` + `rs_kind="meanpool_anom"`（月度层 past-only expanding 去均值再 as-of，泄漏安全；理论支点 [P032]、与 Channel B `anom` 口径一致）。
  - **单 seed**（`compare_rs_anom.py`，seed=42）：`rs`/`finrs` 变差，`m4rep` skill −1.28→+0.05%、CW vs M0 0.894→0.042。
  - **多 seed（5 个：42,0,1,2,3）**（`multiseed_rs_anom.py`；日志 `05_outputs/baselines/Deep/M2_Deep/rs_anom_multiseed.{log,csv}`）：`m4rep` anom>raw 在 **4/5** seed，Δskill 均值 **+0.54%**、方差降低（std 0.70→0.47）；**但** anom 均值 skill 仍 **−0.54%（未过 M0，仅 seed42 过）**、DirAcc 几乎不变（48.5→48.9）、**配对 t p=0.168（不显著）**。
  - **决策**：改善方向一致但幅度小、不显著、且平均仍打不过 M0 → **主结果保留 raw meanpool**（主管线/`deep_metrics.csv` 未变）；**anom-RS 作为补充/稳健性实验保留**（opt-in，写进正文/附录时如实报告"站内异常化让全模型略稳、方差更小，但 RS 净增量仍不显著"）。**暂不升为主模型**；若日后加强（anom+Δ 通道、更多 seed、或仅在 m4rep 臂启用）再复议。
