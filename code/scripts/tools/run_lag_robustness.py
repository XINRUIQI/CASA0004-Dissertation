...
Outputs -> results/experiments/lag_robustness/
  lag_robustness_summary.csv
  matrix_*.csv / baseline_metrics_*.csv

Run:
  python3 code/scripts/tools/run_lag_robustness.py
"""
...
OUT = ROOT / "results" / "_experiments" / "lag_robustness"
LOCKED_GFW = 4
LOCKED_MONTHLY = 5
...
def pull_main(modality: str) -> dict[str, dict]:
    if modality == "M1":
        met = pd.read_csv(
            ROOT / "results/baselines/Flat/M1_Flat/baseline_metrics.csv",
            index_col=0)
        key = "M1_Flat"
    else:
        met = pd.read_csv(
            ROOT / "results/baselines/Flat/M3_Flat/baseline_metrics.csv",
            index_col=0)
        key = "M3_Flat"
    rows = {}
