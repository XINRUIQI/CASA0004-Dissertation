...
def main() -> None:
    ...
    res_m1 = pd.read_csv(m1_flat_predictions(data.ROOT),
    ...
    arms = [
        ("m4_deep_concat", "m4_deep_concat", {}, 104),
        ("m4_deep_gated", "m4_deep_gated", {}, 104),
        ("m4_deep_xattn", "m4_deep_xattn", {}, 104),
        ("m4_deep_gated_drop0.3", "m4_deep_gated", {"modality_dropout": 0.3}, 104),
        ("m4_deep_xattn_drop0.3", "m4_deep_xattn", {"modality_dropout": 0.3}, 104),
        ("m4_deep_gated_longwin(mt78)", "m4_deep_gated", {}, 78),
    ]
    t0 = time.time()
    rows, res_store = [], {}
    for name, cfg, mk, mt in arms:
        res = rolling_origin_deep(dds, CONFIGS[cfg][1], cfg, min_train=mt, seed=42,
                                  epochs=args.epochs, model_kwargs=mk, verbose=False)
        colname = CONFIGS[cfg][1]
    ...
    for name in ("m4_deep_gated", "m4_deep_xattn"):
