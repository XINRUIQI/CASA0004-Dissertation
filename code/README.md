# code — Runbook (reproduction)

Run all commands from the **repository root**. By default, assume `data/processed/` products already exist — this is the fastest path to reproduce the paper’s main tables.

Locked hyperparameters: `docs/appendix/appendix_C_config_EN.md`.  
Software environment: repository `Readme.md` and `code/requirements.txt`.
...
```bash
# Required for Flat
test -f data/processed/merge/outputs/weekly_feature_matrix.csv
test -f data/processed/merge/outputs/weekly_feature_dictionary.csv

# Extra for Deep
test -f data/processed/M3/outputs/m3_graph17_tensors.npz
test -f data/processed/M2/outputs/s2_prithvi_emb_meanpool.npy
test -f data/processed/M2/outputs/s2_prithvi_emb_index.csv
```
...
Shared engine: `code/src/backtest/`. Each modality runs Ridge + XGB; nested layers (M2/3/4) re-run M1 on the same weeks and report DM vs M0 and Clark–West vs M1.
...
python3 code/scripts/flat/run_baseline.py --modality M3              # m3-tier=full (default)
...
**Outputs** → `results/baselines/Flat/M*_Flat/`
...
The default splits the two families because 0 is the neutral value for an anomaly but not for a shipping count. Only RS anomalies and PortWatch carry leading gaps, so `by_family` reproduces `zero` exactly for M1/M2 and `fold_median` exactly for M3; only M4 mixes the two. Every mode scores the same test weeks. `results/experiments/leading_impute/` holds the three-way comparison.

### 3.2 Flat robustness / SHAP / sweep

```bash
python3 code/scripts/flat/M1_Flat/sweep_m1.py
python3 code/scripts/flat/M2_Flat/shap_m2.py
python3 code/scripts/flat/M2_Flat/robustness_m2.py
python3 code/scripts/flat/M2_Flat/sweep_m2.py
python3 code/scripts/flat/M3_Flat/shap_m3.py
python3 code/scripts/flat/M3_Flat/robustness_m3.py
python3 code/scripts/flat/M3_Flat/sweep_m3.py
python3 code/scripts/flat/M4_Flat/shap_m4.py
python3 code/scripts/flat/M4_Flat/robustness_m4.py
python3 code/scripts/flat/M4_Flat/sweep_m4.py
```
...
**Prerequisite**: `results/baselines/Flat/M1_Flat/baseline_predictions.csv` exists (run Flat M1 in §3 first).

```bash
python3 code/scripts/deep/run_deep_baseline.py
# Subset example:
python3 code/scripts/deep/run_deep_baseline.py --modes m3_deep_gated,m4_deep_gated
```
...
```bash
python3 code/scripts/deep/run_deep_fusion_matrix.py          # 3×3 fusion matrix
python3 code/scripts/deep/run_deep_sweep.py                  # seed / lookback / dim, etc.
python3 code/scripts/deep/run_deep_advanced.py               # advanced ablations
python3 code/scripts/deep/run_deep_interpret.py --seeds 42,1,2 --lookback 4
python3 code/scripts/deep/run_deep_interpret_m3.py
python3 code/scripts/deep/run_deep_xattn_viz.py
```
...
# → results/baselines/subperiod/subperiod_summary.csv
```
...
To cross-check reported numbers: open `results/baselines/Flat/*/baseline_metrics*.csv` and `Deep/_cross/deep_metrics.csv`, and compare with Appendix B.

---

## 7. Data rebuild (optional; needs local raw)

`data/raw/` is gitignored. Only if rebuilding features:

```bash
# M1 / M2 / M3 → merge
python3 data/processed/M1/py/build_m1_weekly.py
python3 data/processed/M2/py/build_m2_weekly.py
# python3 data/processed/M2/py/build_m2_weekly.py --watermask   # robustness B4
python3 data/processed/M3/py/aggregate_shipping_to_weekly.py
python3 data/processed/merge/py/build_feature_matrix.py

# Deep extras
python3 data/processed/M3/py/build_m3_graph17.py
# Prithvi embedding export (separate env with foundation-model stack; not this requirements)
# python3 data/processed/M2/py/precompute_s2_embeddings.py
```

Sources: `data/sources.md`.  
Variables and lags: `docs/appendix/appendix_A_data_EN.md`.
...
| Missing merge CSV / graph npz / emb npy | Check `data/processed/`; or rebuild via §7 |
