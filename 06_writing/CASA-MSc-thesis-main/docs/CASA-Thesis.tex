...
% Table/body label in Times, not Latin Modern math (Δ otherwise mismatches RMSE).
\newcommand{\DRMSE}{ΔRMSE}
...
% Keep table notes with the table and prevent mid-note page breaks.
...
I, XINRUI QI, hereby declare that this dissertation is all my own original work and that all sources have been acknowledged. Generative AI has been used for translation, writing refinement and assistance with resolving coding issues. It is \textbf{10,896} words in length.
...
Short-term oil price volatility affects budgeting, hedging and market-risk decisions, yet weekly Brent forecasts remain difficult to improve on a simple no-change rule. Using weekly data from 2019 to 2025, this study examined whether satellite remote sensing and maritime shipping provide predictive information beyond conventional financial indicators. It also tested whether modality-aware representation-level modelling performs better than flat feature fusion. Under a common evaluation framework, each added spatial data was compared with both a finance-only model and the no-change benchmark.
