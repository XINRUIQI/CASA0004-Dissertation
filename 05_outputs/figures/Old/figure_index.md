...
| 3.3 | `fig_3_3_study_sites_map` | 11 个 AOI + 6 个咽喉世界分布图，含波斯湾放大图（不画边） | 3.3 节 |
| 3.4 | `fig_3_4_forecast_origin` | 一次重估起点：训练折 + 嵌套内部验证周、四周输入窗、提前一周目标；区块内沿用参数 | 3.6.1 节 |
| 3.5 | `fig_3_5_deep_architecture` | Deep 三编码器、门控融合（主）与回归头 | 3.5.2 节 |
| 3.6 | `fig_3_6_edge_classes` | 航运图双类边：左为有向周变航次边，右为无向固定走廊边；底部注明 GAT 对称化 | 3.3 节 |
| 3.7 | `fig_3_7_asof_alignment` | 一个周五预测起点上的 as-of 对齐：金融零滞后、EIA/PortWatch +1 周、月度遥感月末+15 日结转 | 3.4.2 节 |
...
python3 04_code/scripts/figures/make_architecture_figure.py   # 图 3.5
python3 04_code/scripts/figures/make_schematic_figures.py     # 图 3.6、3.7
