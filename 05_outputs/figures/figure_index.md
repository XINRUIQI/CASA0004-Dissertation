...
| 4.2 | `fig_4_2_flat_vs_deep_slope` | 匹配模态集上 Flat XGBoost → Deep gated 配对斜率（纵轴反向）；S3 高亮并穿过 M0；S1 为路径参照 | 4.4 节，表 4.3 之后 |
| 4.3 | `fig_4_3_shap_modality` | 仅航运：门控权重 ◇ 与绝对 SHAP 份额 ●（0–60%）；金融为互补项不画；两点不连线 | 4.5 节，表 4.4 之后 |
...
- 图 4.3 只画航运：◇ 门控权重、● 绝对 SHAP 份额；金融 = 100%−航运，不重复绘制。两点不连线，图注写明门控与 SHAP 不可互换。图 4.4–4.5 的量是节点绝对 SHAP 份额，不是门控、不是 GAT 注意力。
...
python3 04_code/scripts/figures/make_result_figures.py          # 附录稳健性图
