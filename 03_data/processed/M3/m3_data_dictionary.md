...
`avail_*` 在合并矩阵中标为 `modality='mask'`，**不作普通预测特征**进入 Ridge/XGB（`04_code/src/backtest/data.py`：targets 与 mask 从不入模型）；用于缺失模态门控 / 长样本诊断。
...
本节说明 M3 原始 CSV 的内容、空间单元含义（咽喉 vs 枢纽），以及三源如何汇入周频矩阵。原始数据均由 Python 脚本自动下载（非手动导出），落盘于 `03_data/raw/03_shipping/`。
...
更新原始数据：在 `03_data/raw/03_shipping/` 对应子目录重跑上述三个 `download_*.py`，再执行 §7 复现命令重建周频矩阵。
...
**项目内交叉引用**：P016 → `01_literature/reading_notes/03 Shipping/P016.md`；PortWatch 方法论 → `external_sources.md` §M3；GFW 下载 → `03_data/raw/03_shipping/GFW/download_gfw_vessel_presence.py`。
...
> 当前建模管线 `04_code/src/backtest/data.py::select_features()` **按 modality 标签选列**，故现状是 **64 列全部进 M3/M4**，靠 Ridge L2 / XGB + 调参隐式降权；如要「仅核心进模型」，需加 `tier` 标签或新增消融臂（见 §10.3）。
...
5. 重跑正式 baseline/sweep/shap（M3、M4）与 `robustness_m3.py`（含新臂）以刷新 `05_outputs/`（本次仅冒烟测试，未覆盖正式产物）。
...
> 构建脚本：`03_data/processed/M3/py/build_m3_graph_weekly.py`（图张量）、`build_emodnet_weekly.py`（EMODnet）。原始来源与采集见 `external_sources.md`「M3 Stage-2」节。最后更新：2026-07-03。
