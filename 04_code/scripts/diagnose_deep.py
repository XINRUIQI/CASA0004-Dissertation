...
layer. Two independent checks that need NO retraining:
  ...
  B. Significance re-examined on the SAVED predictions (deep_predictions.csv):
     for each model, on the common test weeks, report
       ...
       Clark-West vs M0 (random walk) AND vs flat M1_Ridge (the two nested bases),
     ...
     The point: a Clark-West "win" vs the *weak* flat M1 (which is itself worse
...
Run:
  python3 04_code/scripts/diagnose_deep.py
  python3 04_code/scripts/diagnose_deep.py --pred 05_outputs/baselines/deep/deep_predictions.csv
"""
...
# ---------------------------------------------------------------------------
def param_report(lookback: int = 8) -> None:
    ...
    npz = np.load(data.ROOT / "03_data/processed/M3/outputs/m3_graph17_tensors.npz",
                  allow_pickle=True)
...
def pred_report(pred_csv: Path) -> None:
    ...
    base_m1 = "M1_Ridge" if "P_hat_M1_Ridge" in pred.columns else None
    ...
    print("      CWvsM1_p<0.05 only => adds info over the (weak) flat M1, NOT "
          "necessarily over M0.")
...
def main() -> None:
    ...
    ap.add_argument("--pred", default="05_outputs/baselines/deep/deep_predictions.csv")
