...
Prediction sources (all on the same 257 forecast origins)
  Flat        05_outputs/baselines/Flat/M*_Flat/baseline_predictions*.csv
  Deep S1     05_outputs/baselines/Deep/_cross/deep_predictions.csv
  Deep S2-S4  05_outputs/baselines/Deep/_cross/deep_fusion_predictions.csv
              (all nine information-set x fusion cells from one run)

Outputs (-> 05_outputs/tests/):
...
OUT_DIR = data.ROOT / "05_outputs" / "tests"
...
def load_panel() -> pd.DataFrame:
    ...
    if not fus_path.exists():
        raise SystemExit(
            f"Missing {fus_path}.\nRun: python3 "
            f"04_code/scripts/deep/run_deep_fusion_matrix.py")
