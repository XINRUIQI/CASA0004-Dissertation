...
Copies the main matrix/dict into 05_outputs/_experiments/flat_sar_dark/,
...
Run:
  python3 04_code/scripts/tools/run_flat_sar_dark.py
  python3 04_code/scripts/tools/run_flat_sar_dark.py --only m4
"""
...
OUT = ROOT / "05_outputs" / "_experiments" / "flat_sar_dark"
DARK_CSV = ROOT / "03_data/processed/M3/outputs/m3_graph_darkvessel_weekly.csv"
...
def append_locked(records: list, modality: str, n_ship: int) -> None:
    if modality == "M3":
        path = ROOT / "05_outputs/baselines/Flat/M3_Flat/baseline_metrics.csv"
        key = "M3_Flat"
        arm = "locked_M3"
    else:
        path = ROOT / "05_outputs/baselines/Flat/M4_Flat/baseline_metrics_anom.csv"
        key = "M4_Flat"
        arm = "locked_M4"
