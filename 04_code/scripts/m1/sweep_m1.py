...
Outputs (-> 05_outputs/baselines/m1/):
...
Run:
  ...
  python3 04_code/scripts/m1/sweep_m1.py --quick   # coarser retrain, faster
"""
...
SRC_DIR = SCRIPTS_DIR.parent.parent / "src"   # m1/ -> scripts/ -> src/
sys.path.insert(0, str(SRC_DIR))

from backtest import data, metrics, rolling          # noqa: E402

OUT_DIR = data.ROOT / "05_outputs/baselines/m1"
MODEL_NAMES = ["M1_Ridge", "M1_XGB"]
...
def run_one(df, dico, cfg, min_train, val_weeks, seed):
    ...
    res = rolling.rolling_origin(ds, "M1", min_train, cfg["retrain_every"],
...
def run_grid(grid, min_train, val_weeks, seed):
    ...
    for cfg in grid:
        ...
        print(f"  {cfg['name']:18s} feats={res.attrs['n_features']:4d} "
              ...
              f"Ridge={met.loc['M1_Ridge','RMSE']:.3f} "
              f"XGB={met.loc['M1_XGB','RMSE']:.3f} ({dt:.0f}s)")
...
def make_overview(summary, path):
    ...
    rob["L"] = rob["lookback"]
    for name, color in [("M1_Ridge", "tab:blue"), ("M1_XGB", "tab:red")]:
    ...
    w = 0.38
    ridge = [exp[(exp.config == c) & (exp.model == "M1_Ridge")]["RMSE"].iloc[0] for c in exp_order]
    xgb = [exp[(exp.config == c) & (exp.model == "M1_XGB")]["RMSE"].iloc[0] for c in exp_order]
    ax2.bar([i - w / 2 for i in x], ridge, w, color="tab:blue", label="M1_Ridge")
    ax2.bar([i + w / 2 for i in x], xgb, w, color="tab:red", label="M1_XGB")
