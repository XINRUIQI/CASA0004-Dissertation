...
Run:
  python3 04_code/scripts/m4/sweep_m4.py
...
def run_one(df, dico, lookback, min_train, retrain_every, val_weeks, seed):
    ...
    ds_m4   = data.build_dataset(df, cols_m4, lookback, "all")
    res_m4  = rolling.rolling_origin(ds_m4, "M4", min_train, retrain_every,
                                     seed, tune=True, val_weeks=val_weeks)
    ...
    met_m4 = metrics.evaluate(res_m4, ["M4_Ridge", "M4_XGB"])
...
def run_grid(df, dico, min_train, retrain_every, val_weeks, seed):
    ...
    for lb in LOOKBACKS:
        ...
        for mdl in ("Ridge", "XGB"):
            inc = metrics.incremental_tests(res_m4, res_m1, "M4", M1_LABEL, mdl)
            rows.append({
                "lookback":     lb,
                "model":        mdl,
        ...
        print(f"  L={lb}  Ridge={met_m4.loc['M4_Ridge','RMSE']:.3f}  "
              f"XGB={met_m4.loc['M4_XGB','RMSE']:.3f}  ({elapsed:.0f}s)")
