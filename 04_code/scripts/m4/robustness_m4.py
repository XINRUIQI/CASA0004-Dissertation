...
Run:
  ...
  python3 04_code/scripts/m4/robustness_m4.py --quick
"""
...
def select_arm_cols(dico, arm: str) -> list[str]:
    m1 = data.select_features(dico, M1_LABEL)
...
def run_arm(df, dico, arm, min_train, retrain_every, val_weeks, seed):
    ...
    met_arm = metrics.evaluate(res_arm, ["M4_Ridge", "M4_XGB"])
...
def main():
    ...
    for arm in args.arms:
        ...
        for mdl in ("Ridge", "XGB"):
            inc = metrics.incremental_tests(res_arm, res_m1, "M4", M1_LABEL, mdl)
            rows.append({
                "arm":          arm,
                "description":  desc,
                "n_features":   n_cols,
        ...
        print(f"    Ridge={met_arm.loc['M4_Ridge','RMSE']:.3f}  "
              f"XGB={met_arm.loc['M4_XGB','RMSE']:.3f}  ({elapsed:.0f}s)")
