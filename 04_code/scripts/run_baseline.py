...
matrix are selected. Replaces the earlier run_baseline_m0_m1.py + run_baseline_modal.py.
...
What it does for a chosen --modality:
  ...
  2. if it nests M1 (M2/M3/M4), also runs M1 on the SAME weeks and reports the
     ...
     correct nested test, vs M1) -- this is gap B1;
...
M2 feature contract (--m2-features), per channelB_mechanism_plan.md §3/§4:
  anom (default, 55), level (55, C3), all (110), literature (4 NTL_anom, C1 arm).

Window 2019-2026, lookback 4 (locked), target = next-week price trained on the
log return and reconstructed; metrics on the reconstructed price vs M0.

Outputs (-> 05_outputs/baselines/<modality>/, suffix = feature variant for M2/M4):
  baseline_metrics[_<suffix>].csv      M0 + M1 + config Ridge/XGB + Naive (+ DM/CW vs M1)
  baseline_predictions[_<suffix>].csv  per test week, every config's P_hat
  backtest[_<suffix>].png              price tracks + RMSE bars + skill-vs-M0
  baseline_loao[_<suffix>].csv         leave-one-AOI-out table (only with the flag)

  M1 example: 05_outputs/baselines/m1/baseline_metrics.csv
  M2 example: 05_outputs/baselines/m2/baseline_metrics_anom.csv
...
Requires: pandas, numpy, scikit-learn, xgboost, scipy, matplotlib.
"""
...
NESTED_BASE = "M1"                                    # configs nest M1 for CW/DM


def get_out_dir(modality: str) -> Path:
    """Per-modality output subdirectory: baselines/m1/, baselines/m2/, …"""
    return BASE_OUT / modality.lower()
...
# ----------------------------------------------------------------------------
# Summary table (M0 + base M1 + main config + Naive, with CW/DM increments)
# ----------------------------------------------------------------------------
def build_summary(res_main: pd.DataFrame, modality: str,
                  res_base: pd.DataFrame | None) -> pd.DataFrame:
    main_models = [f"{modality}_Ridge", f"{modality}_XGB"]
    m_main = metrics.evaluate(res_main, main_models)

    if res_base is None:                              # modality == M1
        summ = m_main
    ...
    base_models = [f"{NESTED_BASE}_Ridge", f"{NESTED_BASE}_XGB"]
    m_base = metrics.evaluate(res_base, base_models).drop(
        index=["Naive_DirPersist"], errors="ignore")
    # keep M0 once (from base), then base models, then main models, then Naive
    ...
    for model in ("Ridge", "XGB"):
        inc = metrics.incremental_tests(res_main, res_base, modality, NESTED_BASE, model)
        row = f"{modality}_{model}"
...
def leave_one_aoi_out(df, dico, modality, m2_features, res_main, lookback,
    ...
    full = metrics.evaluate(res_main, [f"{modality}_Ridge", f"{modality}_XGB"])
    full_r = float(full.loc[f"{modality}_Ridge", "RMSE"])
    full_x = float(full.loc[f"{modality}_XGB", "RMSE"])
    ...
    for aoi in data.list_aois(dico):
        res = run_config(df, dico, modality, m2_features, lookback, min_train,
                         ...
                         drop_aoi=aoi)
        m = metrics.evaluate(res, [f"{modality}_Ridge", f"{modality}_XGB"])
        r = float(m.loc[f"{modality}_Ridge", "RMSE"])
        x = float(m.loc[f"{modality}_XGB", "RMSE"])
        # dRMSE>0 => dropping this AOI hurts => the AOI carried useful signal
...
def main() -> None:
    ...
    ap.add_argument("--retrain-every", type=int, default=13,
                    help="refit cadence in weeks (locked L4_tuned = 13)")
    ap.add_argument("--feature-mode", choices=["all", "returns"], default="all")
    ap.add_argument("--no-tune", action="store_true", help="disable inner-val tuning")
    ap.add_argument("--val-weeks", type=int, default=52)
    ap.add_argument("--leave-one-aoi-out", action="store_true",
                    help="B3: drop each AOI's RS columns in turn (M2/M4 only)")
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--tag", default=None)
    ap.add_argument("--no-plot", action="store_true")
    ap.add_argument("--matrix", default=None,
                    help="B4: override merged matrix path (relative to merge/outputs/ "
                         "or absolute). E.g. weekly_feature_matrix_watermask.csv")
    ap.add_argument("--dict", default=None,
                    help="B4: override feature dictionary path (auto-paired if omitted "
                         "when --matrix ends with a known suffix like _watermask).")
    args = ap.parse_args()

    tune = not args.no_tune
    has_m2 = "M2" in data.MODALITY_SETS[args.modality]

    # tag = feature-variant suffix only (modality is now encoded by the subdir)
    # M1/M3 → tag="" → files named baseline_metrics.csv
    # M2 anom → tag="anom" → files named baseline_metrics_anom.csv
    if args.tag:
        tag = args.tag
    elif has_m2:
    ...
    # B4: auto-pair dict path from matrix path (e.g. *_watermask → dict *_watermask)
    dict_path = args.dict
    if args.matrix and dict_path is None:
        from pathlib import Path as _P
        mp = _P(args.matrix)
        stem = mp.stem   # e.g. "weekly_feature_matrix_watermask"
    ...
    OUT_DIR = get_out_dir(args.modality)
    ...
    print(f"Config: modality={args.modality} | m2_features="
    ...
    if args.modality != NESTED_BASE:
        ...
        print(f"  {NESTED_BASE:3s}: {res_base.attrs['n_raw']:3d} raw x{args.lookback} "
    ...
    if res_base is not None:
        print("CW_p_vs_M1<0.05: added modality gives SIGNIFICANT nested increment over M1 "
              "(Clark-West, the correct nested test). DM_p_vs_M1 shown for reference.")

    loao_path = None
    if args.leave_one_aoi_out and has_m2:
        print(f"\nLeave-one-AOI-out ({args.modality}, {args.m2_features}):")
        loao = leave_one_aoi_out(df, dico, args.modality, args.m2_features, res_main,
        ...
        print(f"  dRMSE>0 => dropping that AOI hurts => it carried useful signal.")

    if not args.no_plot:
        plot_path = OUT_DIR / f"backtest{suffix}.png"
        make_plot(pred, summ, args.modality, res_base is not None, plot_path,
                  f"  [{args.modality}{suffix}, L={args.lookback}{', tuned' if tune else ''}]")
    else:
        plot_path = None
