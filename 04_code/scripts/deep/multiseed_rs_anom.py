...
epochs=80) and the same flat M0 / M1_Flat_Ridge reference. For each seed and rs_kind
...
Run:
  python3 04_code/scripts/deep/multiseed_rs_anom.py                       # m4rep, 5 seeds
  python3 04_code/scripts/deep/multiseed_rs_anom.py --configs m4rep,finrs --seeds 42,0,1,2,3
"""
...
OUT = deep_out_dir(data.ROOT, "M2") / "rs_anom_multiseed.csv"
...
def main() -> None:
    ...
    ap.add_argument("--configs", default="m4_deep_gated")
        ...
        for seed in seeds:
            for cfg in configs:
                ...
                m = _metrics(merged, col,
                             ...
                             merged["P_hat_M1_Flat_Ridge"].to_numpy(),
    ...
    for cfg in configs:
        ...
        an = (out[(out.config == cfg) & (out.rs_kind == "meanpool_anom")]
              .set_index("seed").sort_index())
        print(f"\n[{cfg}] {CONFIGS[cfg][1]}   per-seed skill vs M0 (%)  /  DirAcc (%)")
