...
retrain_every=13, epochs=80, seed=42). Same flat M0 / M1_Flat_Ridge reference as
...
DirAcc, CW_p vs M0 (nested), DM_p vs M1_Flat_Ridge (non-nested), and the learned
gate alpha (m4rep). Prints a raw-vs-anom delta table + verdict so we can keep the
change only if it helps, else roll back.

Run:
  python3 04_code/scripts/deep/compare_rs_anom.py
  python3 04_code/scripts/deep/compare_rs_anom.py --configs rs,m4rep --epochs 80
"""
...
OUT = deep_out_dir(data.ROOT, "M2") / "rs_anom_compare.csv"
...
def _gate_alpha(ds, epochs, seed) -> dict:
    import torch
    n = len(ds["idx"])
    mods, _, ftype = CONFIGS["m4_deep_gated"]
...
def main() -> None:
    ...
    ap.add_argument("--configs", default="m_rs_deep,m2_deep_gated,m4_deep_gated")
    ...
    for kind in KINDS:
        ...
        for cfg in configs:
            ...
            res[cfg] = rolling_origin_deep(
                ds, CONFIGS[cfg][1], cfg, min_train=args.min_train,
        ...
        ym1 = merged["P_hat_M1_Flat_Ridge"].to_numpy()
        ...
        if "m4_deep_gated" in configs or "m4rep" in configs:
